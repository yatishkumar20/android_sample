...
public class GeofenceBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        SharedPreferences sharedPreferences = context.getSharedPreferences("com.yatish.shhh",
                Context.MODE_PRIVATE);

        //SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        String sound_profile = sharedPreferences.getString("sound", "1");

        /*SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context.getApplicationContext());
        String sound_profile = sharedPreferences.getString("sound_profile", "1");
        String sound_profile = "1";
        Log.e("Yatish",sound_profile);*/
        ...
        if (geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
            if (android.os.Build.VERSION.SDK_INT < 24 ) {
                if(sound_profile.equals("1")) {
                    setRingerMode(context, AudioManager.RINGER_MODE_SILENT);
                }else if(sound_profile.equals("2")){
                    setRingerMode(context, AudioManager.RINGER_MODE_VIBRATE);
                }
            }else if (android.os.Build.VERSION.SDK_INT >= 24 ){
                if(sound_profile.equals("1")) {
                setRingerModeM(context, AudioManager.RINGER_MODE_SILENT);
                }else if(sound_profile.equals("2")){
                setRingerModeM(context, AudioManager.RINGER_MODE_VIBRATE);
                }
            }
            ...
            setRingerMode(context, AudioManager.RINGER_MODE_NORMAL);
                ...
                setRingerModeM(context, AudioManager.RINGER_MODE_NORMAL);
            }
        } else {
            // Log the error.
    ...
    private void sendNotification(Context context, int transitionType) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            builder.setSmallIcon(R.drawable.if_volume_mute);
            builder.setColor(context.getResources().getColor(R.color.trans));
        } else {
            builder.setSmallIcon(R.drawable.if_volume_mute);
        }
    ...
    private void setRingerModeM(Context context, int mode) {
        NotificationManager nm = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
        //nm.setInterruptionFilter(NotificationManager.INTERRUPTION_FILTER_NONE);
        // Check for DND permissions for API 24+
        Log.e("Yatish","Inside"+nm.isNotificationPolicyAccessGranted());
